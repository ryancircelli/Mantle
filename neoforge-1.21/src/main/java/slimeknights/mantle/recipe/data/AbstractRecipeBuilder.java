...
public abstract class AbstractRecipeBuilder<T extends AbstractRecipeBuilder<T>> {
   ...
   * @apiNote  1.20 held an {@link Advancement.Builder} and reached into its criteria map to overwrite
   *           {@code has_the_recipe}. 1.21 made that field an {@code ImmutableMap.Builder}, which has no overwrite and
   *           throws on a duplicate key, so the builder keeps its own map and only assembles the advancement at save.
   *           This is the consumer M2 recorded as REWORK LATER for the {@code Advancement$Builder criteria} transformer,
   *           which is deleted with this class.
   */
