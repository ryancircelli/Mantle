...
public abstract class TagCondition<T> implements ICondition {
  ...
  /**
   * Builds the map codec shared by both conditions, which is the one codec 1.21 needs: a condition's
   * {@link ICondition#codec()} and a {@link net.minecraft.world.level.storage.loot.predicates.LootItemConditionType}
   * both take a {@link MapCodec}, where 1.20 needed a Forge {@code IConditionSerializer} and a loot {@code Serializer}.
   * @param constructor  Builds the condition from its tag
