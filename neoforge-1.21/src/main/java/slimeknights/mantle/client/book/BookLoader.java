...
public class BookLoader implements ResourceManagerReloadListener {
  ...
  public BookLoader() {
    ...
    // ConditionSerializer already exists for the recipe/loot condition registry (data.gson), so the book's own
    // copy of the same Gson adapter was deleted rather than ported - it did nothing ConditionSerializer doesn't
