...
public interface IRecipeHelper {
  ...
  /**
   * Creates an output instance with the added conditions
   * @param output      Base output
   * @param conditions  Extra conditions
   * @return  Wrapped output
   * @apiNote  This is {@link RecipeOutput#withConditions(ICondition...)} under a name Mantle already used. 1.20's
   *           {@code ConsumerWrapperBuilder} is deleted: NeoForge builds the conditional wrapper itself, and the
   *           serializer name override that builder also offered has no 1.21 counterpart, as a recipe's type comes
   *           from the recipe object rather than from the wrapper writing it.
   */
