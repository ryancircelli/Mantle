...
public class Mantle {
  ...
  public Mantle(IEventBus modBus, ModContainer container) {
    ...
    // packets are registered in common setup; the channel itself is not built until
  ...
  private void register(RegisterEvent event) {
    ...
    if (key == Registries.RECIPE_SERIALIZER) {
      ...
      // The recipe serializer pass is arbitrary but early and fires on both dists.
   ...
   * Makes a translation text component for the given name
   * <p>
   * Every argument must be a {@link Component}, {@link Number}, {@link Boolean} or {@link String};
   * {@code TranslatableContents} throws {@link IllegalArgumentException} on anything else. Call {@code toString()}
   * on a resource location, key or path before passing it.
   * @param base  Base name, such as "block" or "gui"
   * @param name  Object name
