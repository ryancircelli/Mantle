...
public class EmptyPotionTransfer extends EmptyFluidContainerTransfer {
  ...
  /** Checks if the given stack is the plain water potion */
  ...
  protected FluidStack getFluid(ItemStack stack) {
    ...
    return TagPreference.getPreference(MantleTags.Fluids.POTION)
      .map(value -> {
        // the potion fluid carries the whole potion as components, same as the 1.20 form carried the whole tag
        FluidStack potion = new FluidStack(value, fluid.getAmount());
        potion.applyComponents(stack.getComponentsPatch());
        return potion;
      })
