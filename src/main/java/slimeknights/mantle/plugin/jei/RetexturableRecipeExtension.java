 ...
 * @apiNote  JEI registers one {@link ICraftingCategoryExtension} per recipe class and hands every method the
 *           {@link RecipeHolder} it applies to, so this is a stateless singleton; the display outputs and the
 *           texture-matching input slots are recomputed per call in {@link #setRecipe}.
 */
