...
public class FluidRenderer {
  ...
  public static void putTexturedQuad(VertexConsumer renderer, Matrix4f matrix, TextureAtlasSprite sprite, Vector3f from, Vector3f to, Direction face, int color, int brightness, int rotation, boolean flowing) {
    ...
    // TextureAtlasSprite#getU and #getV take a 0-1 fraction of the sprite, not a 0-16 pixel coordinate.
    // A flowing sprite is still only half used, hence 0.5 rather than 8.
    ...
    // add quads; VertexConsumer#setColor and #setLight unpack the packed ARGB and lightmap values themselves
    switch (face) {
      case DOWN -> {
  ...
  public static void renderCamera(Minecraft minecraft, PoseStack poseStack, ResourceLocation texture, float opacity, int color) {
    ...
    // Tesselator hands out a live BufferBuilder over a shared buffer and only one may be building at a time,
    // so it is opened here rather than at the top of the method
