...
public class TagPreferenceCommand {
  ...
  private static <T> int runGeneric(CommandContext<CommandSourceStack> context, Registry<T> registry) {
    ...
    if (preference == null) {
      ...
      context.getSource().sendSuccess(() -> Component.translatable(PREFERENCE, registry.key().location().toString(), name.toString(), String.valueOf(registry.getKey(preference))), true);
      return 1;
    }
  }
}
