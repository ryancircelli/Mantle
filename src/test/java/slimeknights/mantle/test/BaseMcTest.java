...
public class BaseMcTest {
   ...
   * DataComponents.CUSTOM_DATA. The helpers below stand in for ItemStack#setTag, #getTag and #hasTag.
   * Note CustomData is copy on write for everyone outside its package, so the two
  ...
  static void setUpRegistries() {
    ...
    // different value, so ask for detection rather than supplying one.
